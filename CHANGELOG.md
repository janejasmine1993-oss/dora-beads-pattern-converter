...
## v0.9.2-lite-high-fidelity-beta-stable - 2026-06-13 (内测稳定版 · 粉丝试用)

### ✅ 高还原像素画 + 高还原编辑器 内测稳定版

**定位**：粉丝试用 / 内测反馈版，不是正式商业发布版。
...
**移除原因**：裁切功能在真实前端验收中导致编辑页白屏，并且拖动交互不稳定（四个调节杆无法被鼠标稳定拖动）。为了保证内测版稳定发布，已暂时移除。后续单独开分支重新设计更稳定的裁切方案。

**验收状态**：
- ✅ npm run build 通过（97 modules）
- ✅ working tree 干净
- ✅ 编辑页不再白屏
- ✅ 所有保留功能联调验收通过
- ✅ 拖动裁切功能已完全移除
...
## v0.9.2-lite-high-fidelity-beta - 2026-06-13 (内测发布版本 - 已回滚)

### 🚀 高还原像素画 + 拖动裁切框 内测版 (ROLLBACK)

**状态**：此版本因拖动裁切功能不稳定，已在 2026-06-13 回滚至 v0.9.2-lite-high-fidelity-beta-stable
...
6. ✅ 25% / 50% / 100% / 200% / Fit 缩放选项
7. ❌ 数值裁切与裁切后导出（已移除）
8. ❌ DOM Overlay 可视化拖动裁切框（已移除）
9. ✅ 右下角「反馈 / 留言」入口
10. ✅ 当前为内测版，反馈功能暂不接数据库

**本版本特性**（已禁用）：
- ❌ 可视化拖动裁切框：四条边拖动不稳定，会导致编辑页白屏
- ❌ 拖动框和数值输入双向同步（已移除）
- ❌ 应用裁切后自动更新图纸参数和导出内容（已移除）
- ✅ 轻量级反馈入口：支持复制反馈内容，预留外部表单链接
- ✅ 所有高还原模式功能完整可用（除裁切外）
...
**验收状态**（已过期）：
- ⚠️ npm run build 通过（98 modules → 97 modules）
- ⚠️ working tree 干净（已在 af01db3 修复）
- ⚠️ 拖动裁切功能不可用（已回滚）

---

## v0.9.1-lite-high-fidelity-editor-checkpoint - 2026-06-13 (当前开发阶段性封存)

### 🎯 高还原像素画模式核心实现 ✅

#### 核心功能
1. **高还原像素画模式**
   - 新增独立的「高还原像素画模式」，与基础图纸模式平行
   - 专为人物照片和高还原需求设计
   - 默认不限制颜色数量，逐格直接匹配品牌色

2. **PixelDesignGrid 设计稿链路**
   - 新增中间数据结构 PixelDesignGrid（像素设计稿）
   - 保留原始采样颜色，无颜色量化压缩
   - 生成后可在预览中查看「设计稿」选项卡
   - 最终 patternData 通过 1:1 映射从 PixelDesignGrid 生成

3. **高还原模式参数优化**
   - 长边上限提升到 1000（基础模式保持 500）
   - 原图比例模式新增「更新尺寸」按钮，快速刷新计算结果
   - 高还原模式显示总格数和性能提示（>500k 格提示）

4. **编辑器缩放优化**
   - 新缩放选项：25% / 50% / 100% / 200% / 适应屏幕
   - 大图（长边 > 500）进入编辑界面默认 fit to screen
   - 自动计算最优缩放比例，避免无法看到完整图纸

5. **图纸区域裁切**
   - 新增数值裁切功能：通过 left/top/right/bottom 精确控制
   - 支持「重置裁切」和「应用裁切」按钮
   - 裁切后自动重新计算 colorStats、总格数、成品尺寸
   - PNG / PDF 导出基于裁切后区域

### 🔧 技术实现细节
- **采样策略**：true pixel-level sampling（average & center）vs 浏览器插值
- **颜色映射**：高还原模式使用 K-means 聚类分析，然后逐格直接匹配品牌色（不聚类压缩）
- **坐标系**：所有裁切操作基于格子坐标，不涉及屏幕像素
- **缩放兼容**：所有功能在各缩放比例下正常工作
- **数据一致性**：pixelDesignGrid 和 patternData 保持 1:1 对应

### ⚠️ 已知未完成项
- **可视化拖动裁切框**：当前仅支持数值输入，不支持在画布上拖动调整裁切框
  - 需要：canvas 拖动逻辑 + 坐标转换 + zoom/fit 适配
  - 预计下一窗口单独处理

### ✅ 验收检查清单
- ✅ npm run build 通过（95 modules）
- ✅ 高还原模式生成图纸正常
- ✅ 像素设计稿预览显示原始采样颜色
- ✅ 格子图显示品牌色号映射
- ✅ 色号图、统计、PNG、PDF 基于 patternData
- ✅ 原图比例模式尺寸计算准确
- ✅ 大图（>500）默认适应屏幕
- ✅ 数值裁切功能完整
- ✅ 基础图纸模式不受影响
...
### 🚀 分支和 Tag
- **分支**：feature/v0.9.1-lite-ratio-color-mode
- **阶段性 Tag**：v0.9.1-lite-high-fidelity-editor-checkpoint
- **状态**：稳定，未合并 main，非生产 tag

---

## v0.9.1-lite-ratio-color-priority - 2026-06-12 (当前开发版本)

### 🎨 新增功能

#### 功能 1：按原图比例尺寸模式
- 新增「按原图比例」尺寸模式，自动根据上传图片的宽高比计算最终图纸尺寸
- 用户输入长边格数，短边自动计算
- 支持上传横图、竖图自动调整
- 裁剪后尺寸计算与最终生成一致
...
#### 功能 2：原图色彩优先匹配模式
- 新增「原图色彩优先」颜色匹配模式
- 绕过颜色量化，直接将像素RGB匹配到当前品牌色卡
- 保留原图色彩观感，同时保证最终输出品牌色号
- 图例、统计、导出全部基于品牌色号（非HEX）

### 🔧 内部改进
- 完整支持尺寸模式和颜色匹配模式的自由组合
- 上传图片后，如已选择「按原图比例」模式，自动重算尺寸
- 切换到「按原图比例」模式时，立即根据当前图片尺寸计算预览
- 裁剪后图片尺寸自动更新，保证比例计算准确
...
**状态**：✅ **已验证可安全停用**  
**成本影响**：每月节省 ¥30-100，年度节省 ¥360-1200  
**恢复难度**：低（代码和文档完整保留）  

**停用前检查结论**：
- ✅ v0.9.0-lite-prod 为纯前端应用，不依赖任何后端或数据库
- ✅ 所有后端相关功能都已通过 LITE_MODE_CONFIG 隐藏
- ✅ 用户数据完全存储在 localStorage（浏览器本地）
- ✅ 网络请求仅包含静态文件，无数据库查询
- ✅ server/ 目录和所有数据库迁移文件已保留
- ✅ 文档齐全，可随时恢复完整版本

**后续维护**：
- CloudBase 后端服务进入暂停维护状态
- PostgreSQL 可停用或缩减规格以降低成本
- 若需恢复完整功能，参考 docs/rollback-v0.9.0-lite.md

详细检查见：docs/postgresql-suspend-v0.9.0-lite-prod.md
